...
ndvi_collection = gee.mask_and_calculate_ndvi(ic)
...
header = "|   Date     |  Mean   | Median  |  Std    |  10%    |  90%    | Pixels  |"
separator = "|------------|---------|---------|---------|---------|---------|---------|"
print(header)
print(separator)
...
def fmt(val):
    return f"{val:7.4f}"
for d, mn, m, s, p1, p9, c in zip(dates_f, means_f, medians_f, stds_f, perc_10_f, perc_90_f, pixels_f):
    print(f"| {d:<10} | {fmt(mn)} | {fmt(m)} | {fmt(s)} | {fmt(p1)} | {fmt(p9)} | {c:7d} |")
